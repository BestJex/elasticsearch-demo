...
import com.jex.elasticsearch.dao.BookRepository;
import com.jex.elasticsearch.dao.PersonRepository;
...
public class UserTests {
    ...
    public void testInsertBooks() {
        bookRepository.save(new Book(42, "A00042", "明史简述", 59, "吴晗", "吴晗背景uniworsity厉害", new Date()));
        bookRepository.save(new Book(43, "A00043", "傅雷家书", 99, "傅聪", "都是NB，class大家u", new Date()));
        bookRepository.save(new Book(24, "A00942", "时间简史", 169, "霍金", "教授宇宙大爆发的59年历史", new Date()));
        bookRepository.save(new Book(25, "A00925", "我的前半生", 39, "方舟89子", "都是生活，每晚9点", new Date()));
        bookRepository.save(new Book(29, "A00029", "围9城", 139, "钱钟书", "你想出城？不存在的", new Date()));
    }
    ...
    public void testSearch() {
        try {
            String searchStr = "时间";
            QueryStringQueryBuilder builder = new QueryStringQueryBuilder(searchStr);
            Iterable<Book> search = bookRepository.search(builder);
    ...
    public void testSearchByPage() {
        try {
            String searchStr = "时间";
            QueryStringQueryBuilder builder = new QueryStringQueryBuilder(searchStr);
            Iterable<Book> search = bookRepository.search(builder, PageRequest.of(0, 2));
            Iterator<Book> iterator = search.iterator();
            while (iterator.hasNext()) {
                System.out.println("--> 数据：" + iterator.next());
            }
